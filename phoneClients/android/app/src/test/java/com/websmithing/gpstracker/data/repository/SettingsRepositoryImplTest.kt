...
package com.websmithing.gpstracker.data.repository // Corrected package
...
import com.websmithing.gpstracker.data.repository.SettingsRepositoryImpl // Correct import
...
class SettingsRepositoryImplTest {
    ...
    private val PREFS_NAME = "com.websmithing.gpstracker.prefs"
    ...
    */ // End commented out test
    ...
    fun `getCurrentSessionId returns correct value from prefs`() = runTest(testDispatcher) {
        ...
        // Test when session ID is empty - should generate new UUID
        clearInvocations(editor)
        whenever(sharedPreferences.getString(eq(KEY_SESSION_ID), eq(""))).thenReturn("")
        val generatedSessionId = repository.getCurrentSessionId()
        
        // Should generate and save a new UUID
        assertNotNull(generatedSessionId)
        assertTrue(generatedSessionId.isNotEmpty())
        val sessionIdCaptor = argumentCaptor<String>()
        verify(editor).putString(eq(KEY_SESSION_ID), sessionIdCaptor.capture())
        verify(editor).apply()
        assertEquals(generatedSessionId, sessionIdCaptor.firstValue)
    }
